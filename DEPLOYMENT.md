...
4. **Environment Variables**
   - **Important**: You CANNOT use the local SQLite file (`file:./dev.db`) on Vercel because the filesystem is ephemeral (read-only/resets).
   - You MUST use a hosted PostgreSQL database (like Vercel Postgres, Neon, or Supabase).
